    ...
    <section className="game-panel p-4">
      ...
      <div className="space-y-2">
        {GENERATORS.map((generator) => {
          ...
          const affordable = state.seconds >= cost;

          return (
            <div key={generator.id} className={`store-row ${affordable ? 'affordable' : 'locked'} ${poppedId === generator.id ? 'popped' : ''}`}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-slate-300">+{generator.baseProduction}/s each</div>
                <div className="mt-1 flex items-center justify-between">
                  <span className="text-sm">Cost: {formatNumber(cost, state.compactNumbers)}s</span>
                  <button
                    className="game-button px-3 py-1 text-xs"
